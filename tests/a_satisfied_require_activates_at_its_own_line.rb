...
#
# zeo satisfies `io/console` and `set` natively and splices `tsort` from a
...
p $LOADED_FEATURES.any? { |f| f.end_with?("tsort.rb") }
require "tsort"
p $LOADED_FEATURES.any? { |f| f.end_with?("tsort.rb") }

# ...and a second require of one records nothing twice.
require "tsort"
p $LOADED_FEATURES.count { |f| f.end_with?("tsort.rb") }

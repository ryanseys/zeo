...
mod tests {
    ...
    fn get_reads_the_live_environment() {
        with_var("SPINEL_TEST_GET", "yes", || {
            let got = env_get(&env_value(), &[s("SPINEL_TEST_GET")], None).unwrap();
    ...
    fn set_then_get_round_trips_and_nil_deletes() {
        let e = env_value();
        env_set(&e, &[s("SPINEL_TEST_SET"), s("v1")], None).unwrap();
        assert_eq!(
            env_get(&e, &[s("SPINEL_TEST_SET")], None).unwrap().to_display_string(),
            "v1"
        );
        // `ENV["X"] = nil` deletes.
        env_set(&e, &[s("SPINEL_TEST_SET"), RubyValue::Nil], None).unwrap();
        assert!(matches!(
            env_get(&e, &[s("SPINEL_TEST_SET")], None).unwrap(),
            RubyValue::Nil
        ));
    }
    ...
    fn fetch_falls_back_to_a_default_then_a_block() {
        let e = env_value();
        let d = env_fetch(&e, &[s("SPINEL_TEST_MISSING"), s("dflt")], None).unwrap();
        ...
        let b = env_fetch(&e, &[s("SPINEL_TEST_MISSING")], Some(blk)).unwrap();
        assert_eq!(b.to_display_string(), "computed:SPINEL_TEST_MISSING");
    }
    ...
    fn fetch_raises_key_error_with_no_default() {
        let r = std::panic::catch_unwind(|| {
            env_fetch(&env_value(), &[s("SPINEL_TEST_ABSENT")], None)
        });
        assert!(r.is_err());
    }
    ...
    fn key_p_and_delete() {
        let e = env_value();
        env_set(&e, &[s("SPINEL_TEST_DEL"), s("x")], None).unwrap();
        assert!(matches!(
            env_key_p(&e, &[s("SPINEL_TEST_DEL")], None).unwrap(),
            RubyValue::Bool(true)
        ));
        let old = env_delete(&e, &[s("SPINEL_TEST_DEL")], None).unwrap();
        ...
        assert!(matches!(
            env_delete(&e, &[s("SPINEL_TEST_DEL")], None).unwrap(),
            RubyValue::Nil
        ));
    }
    ...
    fn to_h_and_keys_see_a_set_variable() {
        with_var("SPINEL_TEST_TOH", "1", || {
            ...
            assert!(names.contains(&"SPINEL_TEST_TOH".to_string()));
    ...
    fn each_yields_every_pair() {
        with_var("SPINEL_TEST_EACH", "v", || {
            ...
            assert!(seen.lock().contains(&"SPINEL_TEST_EACH".to_string()));
        });
    }

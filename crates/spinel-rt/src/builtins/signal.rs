...
//! `Signal.signame`, and `Signal.trap`. spinel installs no OS signal handlers,
...
/// The Darwin/BSD signal set spinel targets, canonical name (no `SIG` prefix)
...
thread_local! {
    ...
    /// spinel installs no real handlers, so this only feeds `trap`'s return
...
/// never set). A no-op beyond that bookkeeping -- spinel installs no handler, so

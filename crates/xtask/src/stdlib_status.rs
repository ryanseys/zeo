...
//! COMPILE it -- front end plus Cranelift emission only (via `zeo --emit-clif`; no linker, no
...
/// Compiles one file with `zeo <file> -I <lib-dir> --emit-clif=/dev/null`
...
fn classify(zeo: &Path, lib_dir: &Path, file: &Path, budget: crate::jobs::Budget) -> Status {
    ...
    // `--emit-clif=/dev/null`: this reads the exit status and stderr and never
    // the program, so it wants the cheapest full compile there is -- front end
    // plus emission, with the output thrown away.
    cmd.arg(file)
        .arg("-I")
        .arg(lib_dir)
        .arg("--emit-clif=/dev/null");

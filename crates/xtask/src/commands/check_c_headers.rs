...
const USAGE: &str = "\
usage: cargo xtask check-c-headers <subcommand> [options]
    ...
    if std::fs::read_to_string(&mkmf).ok().as_deref() != Some(&upstream) {
        if check {
            return Err(Error::new(format!(
                "{MKMF_RB} is not upstream {}'s lib/mkmf.rb -- run `cargo xtask check-c-headers hunks`",
                pin.tag
            )));
        }
...
fn cmd_api(check: bool) -> Result<(), Error> {
    ...
    if check {
        ...
        if !drift.is_empty() {
            return Err(Error::new(format!(
                "stale -- run `cargo xtask check-c-headers api`: {}",
                drift.join(", ")
            )));
        }
        println!(
...
//!
//! Generated by `cargo xtask check-c-headers api` from clang's own AST of the vendored
//! headers. Do not edit.
//!
...
//!
//! Generated by `cargo xtask check-c-headers api`. Do not edit.
//!
...
fn cmd_forward(check: bool, reverify: bool) -> Result<(), Error> {
    ...
    if check {
        if read(&root_join(FORWARD_RS))? != rs {
            return Err(Error::new(format!(
                "stale -- run `cargo xtask check-c-headers forward`: {FORWARD_RS}"
            )));
        }

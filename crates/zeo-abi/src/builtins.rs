...
pub const BUILTINS: &[BuiltinClass] = &[
    ...
    // Ids 102-104 belonged to the native gzip classes; these three rows moved
    // down from the table's tail to keep it contiguous when those left.
    BuiltinClass {
    ...
    // The digest framework's ancestry, ruby's own shape. The method surface
    // is the shared algorithm table below these plus the gem's vendored Ruby
    // half, which reopens `Class` and `Instance`.
    BuiltinClass {
        id: DIGEST_INSTANCE_MODULE,
    ...
    BuiltinClass {
        id: DIGEST_CLASS_CLASS,
        name: "Digest::Class",
        is_module: false,
        superclass: Some(OBJECT_CLASS),
        includes: &[DIGEST_INSTANCE_MODULE],
        feature: Some("digest"),
    },
    BuiltinClass {
        id: DIGEST_BASE_CLASS,
        name: "Digest::Base",
        is_module: false,
        superclass: Some(DIGEST_CLASS_CLASS),
        includes: &[],

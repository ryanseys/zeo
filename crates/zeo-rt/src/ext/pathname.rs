...
builtin_methods! {
    ...
    "to_s" | "to_path" | "to_str" => fn to_s(recv, args, _b) { arity!(args, 0); Ok(str_val(recv_path(recv).to_string())) }
    "inspect" => fn inspect(recv, args, _b) { arity!(args, 0); Ok(str_val(format!("#<Pathname:{}>", recv_path(recv)))) }
    "freeze" => fn freeze(recv, args, _b) { arity!(args, 0); Ok(recv.clone()) }
    "frozen?" => fn frozen_p(_recv, args, _b) { arity!(args, 0); Ok(RubyValue::Bool(true)) }

    // `Pathname#+`/`#/`/`#join` resolve `.`/`..` lexically at the join (CRuby's
    // `plus`), so the result is cleaned -- unlike raw `File.join`.
    "+" | "/" => fn plus(recv, args, _b) {
        arity!(args, 1);
        Ok(pathname_val(cleanpath(&join_two(recv_path(recv), &arg_path(&args[0])?))))
    }
    "join" => fn join(recv, args, _b) {
        let mut acc = recv_path(recv).to_string();
        for a in args { acc = join_two(&acc, &arg_path(a)?); }
        Ok(pathname_val(cleanpath(&acc)))
    }
    "basename" => fn basename(recv, args, _b) {
        arity!(args, 0..=1);
    ...
    "dirname" | "parent" => fn dirname(recv, args, _b) { arity!(args, 0); Ok(pathname_val(dirname_of(recv_path(recv)))) }
    "extname" => fn extname(recv, args, _b) { arity!(args, 0); Ok(str_val(extname_of(recv_path(recv)))) }
    "expand_path" => fn expand_path(recv, args, _b) {
        arity!(args, 0..=1);
    ...
    "cleanpath" => fn cleanpath_m(recv, args, _b) { arity!(args, 0..=1); Ok(pathname_val(cleanpath(recv_path(recv)))) }
    "sub_ext" => fn sub_ext(recv, args, _b) {
        arity!(args, 1);
    ...
    "relative_path_from" => fn relative_path_from_m(recv, args, _b) {
        arity!(args, 1);
        Ok(pathname_val(relative_path_from(recv_path(recv), &arg_path(&args[0])?)?))
    }

    "absolute?" => fn absolute_p(recv, args, _b) { arity!(args, 0); Ok(RubyValue::Bool(recv_path(recv).starts_with('/'))) }
    "relative?" => fn relative_p(recv, args, _b) { arity!(args, 0); Ok(RubyValue::Bool(!recv_path(recv).starts_with('/'))) }
    "root?" => fn root_p(recv, args, _b) {
        arity!(args, 0);
        let p = recv_path(recv);
    ...
    "exist?" => fn exist_p(recv, args, _b) { arity!(args, 0); Ok(RubyValue::Bool(Path::new(recv_path(recv)).exists())) }
    "directory?" => fn directory_p(recv, args, _b) { arity!(args, 0); Ok(RubyValue::Bool(Path::new(recv_path(recv)).is_dir())) }
    "file?" => fn file_p(recv, args, _b) { arity!(args, 0); Ok(RubyValue::Bool(Path::new(recv_path(recv)).is_file())) }
    "symlink?" => fn symlink_p(recv, args, _b) { arity!(args, 0); Ok(RubyValue::Bool(std::fs::symlink_metadata(recv_path(recv)).map(|m| m.file_type().is_symlink()).unwrap_or(false))) }

    "read" => fn read(recv, args, _b) {
        arity!(args, 0..=2);
        let p = recv_path(recv);
        Ok(str_val(std::fs::read_to_string(p).map_err(|e| io_err(p, &e))?))
    }
    "binread" => fn binread(recv, args, _b) {
        arity!(args, 0..=1);
        let p = recv_path(recv);
        let bytes = std::fs::read(p).map_err(|e| io_err(p, &e))?;
        Ok(str_val(String::from_utf8_lossy(&bytes).into_owned()))
    }
    "write" => fn write(recv, args, _b) {
        arity!(args, 1);
        let p = recv_path(recv);
...
builtin_methods! {
    ...
    "new" => fn new_m(_recv, args, _b) {
        arity!(args, 1);
        Ok(pathname_val(arg_path(&args[0])?))
    }
    "getwd" | "pwd" => fn getwd(_recv, args, _b) {
        arity!(args, 0);
        let cwd = std::env::current_dir().map(|p| p.to_string_lossy().into_owned()).unwrap_or_default();
        Ok(pathname_val(cwd))
    }
    "glob" => fn glob(_recv, args, _b) {
        arity!(args, 1..=2);

...
ruby_class! {
    ...
    def "+" | "/" (recv, other) {
        Ok(pathname_val(cleanpath(&join_two(recv_path(recv), &arg_path(other)?))))
    }
    ...
    def "dirname" | "parent" (recv) { Ok(pathname_val(dirname_of(recv_path(recv)))) }
    def "extname" (recv) { Ok(str_val(extname_of(recv_path(recv)))) }
    def "expand_path" (recv, arg?) {
        let base = match arg {
            None | Some(RubyValue::Nil) => None,
            Some(v) => Some(arg_path(v)?),
        };
    ...
    def "relative_path_from" (recv, arg) {
        Ok(pathname_val(relative_path_from(recv_path(recv), &arg_path(arg)?)?))
    }

    def "absolute?" (recv) { Ok(RubyValue::Bool(recv_path(recv).starts_with('/'))) }
    def "relative?" (recv) { Ok(RubyValue::Bool(!recv_path(recv).starts_with('/'))) }
    def "root?" (recv) {
        let p = recv_path(recv);
    ...
    def "exist?" (recv) { Ok(RubyValue::Bool(Path::new(recv_path(recv)).exists())) }
    def "directory?" (recv) { Ok(RubyValue::Bool(Path::new(recv_path(recv)).is_dir())) }
    def "file?" (recv) { Ok(RubyValue::Bool(Path::new(recv_path(recv)).is_file())) }
    def "symlink?" (recv) { Ok(RubyValue::Bool(std::fs::symlink_metadata(recv_path(recv)).map(|m| m.file_type().is_symlink()).unwrap_or(false))) }

    def "read" (recv, _arg1?, _arg2?) {
        let p = recv_path(recv);
        Ok(str_val(std::fs::read_to_string(p).map_err(|e| io_err(p, &e))?))
    }
    def "binread" (recv, _arg?) {
        let p = recv_path(recv);
        let bytes = std::fs::read(p).map_err(|e| io_err(p, &e))?;
        Ok(str_val(String::from_utf8_lossy(&bytes).into_owned()))
    }
    def "write" cfunc (recv, arg) {
        let p = recv_path(recv);
        let content = arg_path(arg)?;
    ...
    def self."new" cfunc (_recv, arg) {
        Ok(pathname_val(arg_path(arg)?))
    }
    def self."getwd" | "pwd" (_recv) {
        let cwd = std::env::current_dir().map(|p| p.to_string_lossy().into_owned()).unwrap_or_default();
        Ok(pathname_val(cwd))
    }
    def self."glob" cfunc (_recv, arg1, _arg2?) {
        let pattern = arg_path(arg1)?;

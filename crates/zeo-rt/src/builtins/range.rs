...
ruby_class! {
    ...
    def self."new" allocs cfunc inherits (_recv, first, last, exclude_end?, &_block) {
        let excl = exclude_end.is_some_and(|v| v.truthy());
        crate::range_checked(
            crate::range_endpoint(first.clone()),
            crate::range_endpoint(last.clone()),
            excl,
        )
    }

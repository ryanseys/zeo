...
fn syserr(code: c_int, what: Option<&str>) -> Signal {
    ...
    match what {
        Some(w) if !w.is_empty() => crate::dispatch::raise_error(class, format!("{desc} - {w}")),
        _ => crate::dispatch::raise_error(class, desc),
    }
}
...
crate::cext_fn! {
    ...
    fn rb_interrupt() -> () {
        Err(crate::dispatch::raise_error("Interrupt", String::new()))
    }
    ...
    fn rb_error_arity(argc: c_int, min: c_int, max: c_int) -> () {
        ...
        Err(crate::dispatch::wrong_arity(argc.max(0) as usize, &want))
    }
    ...
    fn zeo_cext_warn(msg: *const c_char, verbose_only: c_int, category: c_int) -> () {
        if verbose_only != 0
            && !matches!(crate::globals::global_get(0, "$VERBOSE"), RubyValue::Bool(true))
        {
            return Ok(());
        }
        if let Some(cat) = category_name(category)
            && !crate::builtins::warning::category_enabled(cat)
        {
            return Ok(());
        }
        crate::builtins::warning::rb_warn(&unsafe { cstr(msg) });
        Ok(())
    }
    ...
    fn zeo_cext_sys_warning(msg: *const c_char, code: c_int) -> () {
        if !matches!(crate::globals::global_get(0, "$VERBOSE"), RubyValue::Bool(true)) {
            return Ok(());
        }
        let (_, desc) = errno_row(code);
        crate::builtins::warning::rb_warn(&format!("{}: {desc}", unsafe { cstr(msg) }));
        Ok(())
    }
...
fn name_error(class: &str, msg: &str, name: RubyValue) -> Result<Signal, Signal> {
    let Some(cls) = crate::constants::const_get(zeo_abi::OBJECT_CLASS.0, class) else {
        return Ok(crate::dispatch::raise_error(class, msg.to_string()));
    };
    let text = crate::builtins::string::str_value_in_enc(crate::encoding::UTF_8, msg);
...
fn tagged_value(module: &RubyValue, sig: Signal) -> Result<Signal, Signal> {
    ...
    crate::runtime_meta::runtime_extend(&exc, module)?;
    Ok(Signal::Raise(exc))
}

...
pub(super) fn send(recv: &RubyValue, meth: &str, args: &[RubyValue]) -> Result<RubyValue, Signal> {
    crate::dispatch::send_value(recv, Symbol::intern(meth), args, None)
}
...
fn const_lookup(owner: ClassId, name: &str, scope: Scope) -> Option<RubyValue> {
    match scope {
        Scope::Own => crate::constants::const_get_own(owner.0, name),
        // `const_get_scoped` is `::`'s search: ancestors, and NOT Object.
        Scope::From => crate::constants::const_get_scoped(owner.0, name),
        Scope::Full => crate::constants::const_get(owner.0, name),
    }
}
...
fn real_class(cid: ClassId) -> ClassId {
    crate::runtime_meta::singleton_class_owner(cid)
        .and_then(|owner| crate::dispatch::class_name(owner).map(|_| owner))
...
crate::cext_fn! {
    ...
    fn rb_obj_call_init(obj: Value, argc: c_int, argv: *const Value) -> () {
        ...
        let args = unsafe { args_of(argc, argv) };
        crate::dispatch::send_value(&recv, Symbol::intern("initialize"), &args, None)?;
        Ok(())
    }
    ...
    fn rb_obj_init_copy(dst: Value, src: Value) -> Value {
        ...
        crate::dispatch::send_value(&d, Symbol::intern("initialize_copy"), &[s], None)?;
        Ok(dst)
    }
    ...
    fn rb_obj_is_method(v: Value) -> Value {
        let recv = unsafe { value_of(v) };
        let is = crate::dispatch::class_name(recv.class_id()).is_some_and(|n| n == "Method");
        Ok(super::convert::boolean(is))
    }

    fn rb_obj_is_fiber(v: Value) -> Value {
        let recv = unsafe { value_of(v) };
        let is = crate::dispatch::class_name(recv.class_id()).is_some_and(|n| n == "Fiber");
        Ok(super::convert::boolean(is))
    }
    ...
    fn rb_module_new() -> Value {
        to_value(&crate::runtime_meta::runtime_module_new(None)?)
    }
    ...
    fn rb_class_new(superclass: Value) -> Value {
        ...
        to_value(&crate::runtime_meta::runtime_class_new(Some(sup), None)?)
    }
    ...
    fn rb_class_path_cached(klass: Value) -> Value {
        let cid = unsafe { as_class(klass)? };
        to_value(&match crate::dispatch::class_name(cid) {
            Some(n) => crate::builtins::string::str_value_in_enc(crate::encoding::UTF_8, &n),
            None => RubyValue::Nil,
        })
    }
    ...
    fn rb_const_remove(m: Value, id: Id) -> Value {
        let owner = unsafe { as_class(m)? };
        let name = symbol_of(id).name_str().to_string();
        let gone = crate::constants::const_remove(owner.0, &name)
    ...
    fn rb_define_global_const(name: *const c_char, val: Value) -> () {
        ...
        crate::constants::const_set(zeo_abi::OBJECT_CLASS.0, &name, unsafe { value_of(val) });
        Ok(())
    }
    ...
    fn rb_iv_get(obj: Value, name: *const c_char) -> Value {
        ...
        to_value(&crate::dispatch::instance_variable_get(&recv, &n)?)
    }

    fn rb_iv_set(obj: Value, name: *const c_char, val: Value) -> Value {
        let recv = unsafe { value_of(obj) };
        let n = ivar_name(&unsafe { cstr(name) });
        crate::dispatch::instance_variable_set(&recv, &n, unsafe { value_of(val) })?;
        Ok(val)
    }
    ...
    fn rb_ivar_defined(obj: Value, id: Id) -> Value {
        ...
        Ok(super::convert::boolean(crate::dispatch::ivar_defined(&recv, &bare)))
    }
    ...
    fn rb_ivar_foreach(
        ...
        for name in names {
            let val = crate::dispatch::instance_variable_get(&recv, &name)?;
...
fn resolve_path(path: &str) -> Result<RubyValue, Signal> {
    ...
    for seg in path.split("::").filter(|s| !s.is_empty()) {
        let v =
            crate::constants::const_get(owner.0, seg).ok_or_else(|| missing_const(owner, seg))?;
        match &v {
...
fn try_convert(recv: &RubyValue, meth: &str) -> Option<RubyValue> {
    ...
    if !crate::dispatch::responds_to_value(recv, sym, true) {
        return None;
    }
    crate::dispatch::send_value(recv, sym, &[], None).ok()
}

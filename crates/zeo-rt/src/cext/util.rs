...
fn env_call(meth: &str, args: &[String]) -> Result<(), crate::Signal> {
    let Some(env) = crate::constants::const_get(zeo_abi::OBJECT_CLASS.0, "ENV") else {
        return Ok(());
    };
    let vals: Vec<crate::RubyValue> = args
        .iter()
        .map(|a| crate::builtins::string::str_value_in_enc(crate::encoding::UTF_8, a))
        .collect();

...
crate::cext_fn! {
    ...
    fn rb_define_method_id(klass: Value, id: Id, f: super::method::MethodPtr, argc: c_int) -> () {
        ...
        crate::runtime_meta::runtime_define_method(owner, symbol_of(id), body)?;
        Ok(())
    }
    ...
    fn rb_define_module_function(
        ...
        let body = unsafe { super::method::method_proc(f, argc)? };
        crate::runtime_meta::runtime_define_method(owner, sym, body)?;
        crate::runtime_meta::runtime_module_function(owner, &[RubyValue::Symbol(sym)])?;
        Ok(())
    }
    ...
    fn rb_define_global_function(
        ...
        // SAFETY: the caller promised the shape.
        let body = unsafe { super::method::method_proc(f, argc)? };
        crate::runtime_meta::runtime_define_method(zeo_abi::OBJECT_CLASS, sym, body)?;
    ...
    fn rb_alias(klass: Value, new: Id, old: Id) -> () {
        let owner = unsafe { as_class(klass)? };
        crate::runtime_meta::runtime_alias_method(owner, symbol_of(new), symbol_of(old))?;
        Ok(())
    }

    fn rb_undef(klass: Value, id: Id) -> () {
        let owner = unsafe { as_class(klass)? };
        crate::runtime_meta::runtime_undef_method(owner, &[RubyValue::Symbol(symbol_of(id))])?;
        Ok(())
    }
    ...
    fn rb_undef_method(klass: Value, name: *const c_char) -> () {
        let owner = unsafe { as_class(klass)? };
        let sym = Symbol::intern(&unsafe { cstr(name) });
        crate::runtime_meta::runtime_undef_method(owner, &[RubyValue::Symbol(sym)])?;
        Ok(())
    }
    ...
    fn rb_remove_method(klass: Value, name: *const c_char) -> () {
        let owner = unsafe { as_class(klass)? };
        let sym = Symbol::intern(&unsafe { cstr(name) });
        crate::runtime_meta::runtime_remove_method(owner, &[RubyValue::Symbol(sym)])?;
        Ok(())
    }

    fn rb_remove_method_id(klass: Value, id: Id) -> () {
        let owner = unsafe { as_class(klass)? };
        crate::runtime_meta::runtime_remove_method(owner, &[RubyValue::Symbol(symbol_of(id))])?;
        Ok(())
    }
    ...
    fn rb_prepend_module(target: Value, module: Value) -> () {
        ...
        crate::runtime_meta::runtime_prepend(&t, &[m])?;
        Ok(())
    }
    ...
    fn rb_attr_get(obj: Value, id: Id) -> Value {
        ...
        to_value(&crate::dispatch::instance_variable_get(&recv, &name).unwrap_or(RubyValue::Nil))
    }
    ...
    fn rb_alias_variable(new: Id, old: Id) -> () {
        crate::globals::global_alias(0, symbol_of(new).name_str(), symbol_of(old).name_str());
        Ok(())
    }
    ...
    fn rb_reg_new(p: *const c_char, len: c_long, options: c_int) -> Value {
        ...
        let src = crate::builtins::string::str_value_in_enc(
    ...
    fn rb_range_values(v: Value, beg: *mut Value, end: *mut Value, excl: *mut c_int) -> c_int {
        ...
        if crate::dispatch::class_name(r.class_id()).as_deref() != Some("Range") {
            return Ok(0);
        }
        let b = send(&r, "begin", &[])?;
        let e = send(&r, "end", &[])?;
...
fn name_and_home(v: Value, home: crate::dispatch::ClassId, name: &str) -> Result<Value, Signal> {
...
fn install_name(cid: crate::dispatch::ClassId, home: crate::dispatch::ClassId, name: &str) {
    ...
    crate::constants::const_set(home.0, name, RubyValue::Class(cid));
}
...
fn register_slot(
    ...
    if !slot.is_null() {
        ...
        crate::globals::global_set(0, &full, unsafe { value_of(held) });
    }
    Ok(())
}

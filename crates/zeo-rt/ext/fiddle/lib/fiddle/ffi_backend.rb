...
module Fiddle
  # zeo: upstream answers a Fiddle::Pointer here; CRuby's fiddle returns the
  # raw ADDRESS Integer of a real libc malloc (later handed to Fiddle.free),
  # which is what callers observe.
  def self.malloc(size)
    Fiddle::Pointer::LibC::MALLOC.call(size).to_i
  end

  # zeo: wrap an Integer address back to a pointer for the call (upstream
  # relies on attach_function's custom-type marshaling accepting Integers).
  def self.free(ptr)
    ptr = Fiddle::Pointer.new(ptr) unless ptr.is_a?(Fiddle::Pointer)
  ...
  module Types
    ...
    INT8_T       = CHAR
    UINT8_T      = UCHAR
    # zeo: upstream sizes these via `if FFI::Type::Builtin::SHORT.size == 2`
    # chains at module-body level; zeo compiles for LP64 only, so the chains
    # collapse to their LP64 arms (the same values CRuby's fiddle reports).
    INT16_T      = SHORT
    UINT16_T     = USHORT
    INT32_T      = INT
    UINT32_T     = UINT
  ...
  module FFIBackend
    ...
    # zeo: upstream resolves a Symbol via `Types.const_get(sym.to_s.upcase)`;
    # a computed const_get can't resolve under AOT compilation, so the Symbol
    # spellings are an explicit table.
    SYMBOL_TYPES = {
      void: Types::VOID, voidp: Types::VOIDP, char: Types::CHAR,
      uchar: Types::UCHAR, short: Types::SHORT, ushort: Types::USHORT,
      int: Types::INT, uint: Types::UINT, long: Types::LONG,
      ulong: Types::ULONG, long_long: Types::LONG_LONG,
      ulong_long: Types::ULONG_LONG, float: Types::FLOAT,
      double: Types::DOUBLE, variadic: Types::VARIADIC,
      const_string: Types::CONST_STRING, bool: Types::BOOL,
      int8_t: Types::INT8_T, uint8_t: Types::UINT8_T,
      int16_t: Types::INT16_T, uint16_t: Types::UINT16_T,
      int32_t: Types::INT32_T, uint32_t: Types::UINT32_T,
      int64_t: Types::INT64_T, uint64_t: Types::UINT64_T,
      ssize_t: Types::SSIZE_T, size_t: Types::SIZE_T,
      ptrdiff_t: Types::PTRDIFF_T, intptr_t: Types::INTPTR_T,
      uintptr_t: Types::UINTPTR_T,
    }

    def self.to_ffi_type(fiddle_type)
      if fiddle_type.is_a?(Symbol)
        fiddle_type = SYMBOL_TYPES.fetch(fiddle_type) do
          raise TypeError.new("cannot convert #{fiddle_type} to ffi")
        end
      end
      ...
      # zeo: upstream raises TypeError "cannot convert ... to ffi"; CRuby's
      # fiddle raises RuntimeError "unknown type N", which callers observe.
      raise "unknown type #{fiddle_type}" unless ffi_type
      ffi_type
    end
  end
  ...
  class Function
    ...
    def initialize(ptr, args, return_type, abi = DEFAULT, kwargs = nil)
      ...
      # zeo: upstream says "invalid argument types"; CRuby's fiddle raises
      # this message.
      raise TypeError.new "wrong argument type #{args.class} (expected Array)" unless args.is_a?(Array)
  ...
  class Pointer
    ...
    def []=(*args, value)
      ...
      # zeo: upstream says "NULL pointer access"; CRuby's fiddle raises
      # "NULL pointer dereference" for a NULL []= as it does for [].
    ...
    # zeo: upstream binds these with `extend FFI::Library` + `attach_function`,
    # whose return value it keeps as callable constants; zeo's attach_function
    # is a compile-time extern emission with no value object, so the same
    # bindings are built explicitly over DynamicLibrary + Function.
    module LibC
      libc = FFI::DynamicLibrary.open(
        nil, FFI::DynamicLibrary::RTLD_LAZY | FFI::DynamicLibrary::RTLD_GLOBAL
      )
      MALLOC = FFI::Function.new(
        FFI::Type::Builtin::POINTER, [FFI::Type::Builtin::ULONG],
        libc.find_function("malloc")
      )
      REALLOC = FFI::Function.new(
        FFI::Type::Builtin::POINTER,
        [FFI::Type::Builtin::POINTER, FFI::Type::Builtin::ULONG],
        libc.find_function("realloc")
      )
      FREE = FFI::Function.new(
        FFI::Type::Builtin::VOID, [FFI::Type::Builtin::POINTER],
        libc.find_function("free")
      )
    end
    ...
    def self.malloc(size, free = nil)
      ...
      # zeo: `LibC.malloc` was the attach_function-defined module function;
      # the rebuilt binding is called through its constant.
      pointer = new(LibC::MALLOC.call(size), size, free)
      if block_given?
        begin
          yield(pointer)
        ensure
    ...
    def to_s(len = nil)
      # zeo: upstream turns the NULL read into DLError "NULL pointer access";
      # CRuby's fiddle raises ArgumentError "NULL pointer given" here.
      raise ArgumentError, "NULL pointer given" if ffi_ptr.null?
      if len
    ...
    def eql?(other)
      # zeo: upstream answers nil for a non-Pointer; CRuby's fiddle answers
      # false.
  ...
  class Handle
    ...
    def initialize(libname = nil, flags = RTLD_LAZY | RTLD_GLOBAL)
      ...
      rescue LoadError, RuntimeError => e # LoadError for JRuby, RuntimeError for TruffleRuby
        # zeo: upstream says "Could not open #{libname}"; CRuby's fiddle
        # surfaces the raw dlerror(3) text, which zeo's LoadError carries.
    ...
    def sym(func)
      ...
      # zeo: upstream interpolates the bare name; CRuby's fiddle quotes it
      # (`unknown symbol "strlen"`).
      raise DLError.new("unknown symbol #{func.inspect}") if address.nil? || address.null?
      address.to_i
    end

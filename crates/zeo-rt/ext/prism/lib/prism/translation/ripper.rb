...
module Prism
  module Translation
    ...
    class Ripper < Compiler
      # zeo: CRuby's Ripper reads its source line by line from anything with
      # `gets`, or through `StringValue`, so an object with `to_str` works.
      # prism's parser takes only a String. This converts the way Ripper does.
      def self.source_string(src) # :nodoc:
        return src if src.is_a?(String)
        return String.new(src) unless src.respond_to?(:gets)
      ...
      def self.lex(src, filename = "-", lineno = 1, raise_errors: false)
        result = Prism.lex_compat(source_string(src), filepath: filename, line: lineno, version: "current")
      ...
      def initialize(source, filename = "(ripper)", lineno = 1)
        @source = Ripper.source_string(source)
        @filename = filename
        @lineno = lineno
        @column = 0
        @result = nil

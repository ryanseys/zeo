...
crate::cext_fn! {
    ...
    fn rb_block_call(
        ...
        to_value(&zeo_rt::dispatch::send_value(&r, symbol_of(mid), &args, block)?)
    }
    ...
    fn rb_each(v: Value) -> Value {
        ...
        to_value(&zeo_rt::dispatch::send_value(&recv, Symbol::intern("each"), &[], block)?)
    }
    ...
    fn rb_apply(recv: Value, mid: Id, args: Value) -> Value {
        ...
        to_value(&zeo_rt::dispatch::send_value(&r, symbol_of(mid), &list, None)?)
    }
    ...
    fn rb_check_funcall(recv: Value, mid: Id, argc: c_int, argv: *const Value) -> Value {
        ...
        if !zeo_rt::dispatch::responds_to_value(&r, name, true) {
            return Ok(value::Q_UNDEF);
        }
        let args = unsafe { args_of(argc, argv) };
        to_value(&zeo_rt::dispatch::send_value(&r, name, &args, None)?)
    }
    ...
    fn rb_call_super(argc: c_int, argv: *const Value) -> Value {
        ...
        let recv = super::call::current_receiver()
            .unwrap_or_else(zeo_rt::dispatch::main_object);
        to_value(&zeo_rt::runtime_meta::send_super_dynamic(&recv, &args, super::call::current_block())?)
    }
    ...
    fn rb_block_proc() -> Value {
        match super::call::current_block() {
            Some(b) => to_value(&b),
            None => Err(zeo_rt::builtins::arg_error!("tried to create Proc object without a block")),
        }
    }
    ...
    fn rb_need_block() -> () {
        if super::call::current_block().is_none() {
            return Err(zeo_rt::builtins::local_jump_error!("no block given (yield)"));
        }
        Ok(())
    }
    ...
    fn rb_proc_call(p: Value, args: Value) -> Value {
        ...
        to_value(&zeo_rt::dispatch::send_value(&proc, Symbol::intern("call"), &list, None)?)
    }
    ...
    fn rb_method_call(argc: c_int, argv: *const Value, m: Value) -> Value {
        ...
        let args = unsafe { args_of(argc, argv) };
        to_value(&zeo_rt::dispatch::send_value(&meth, Symbol::intern("call"), &args, None)?)
    }
    ...
    fn rb_method_call_with_block(argc: c_int, argv: *const Value, m: Value, block: Value) -> Value {
        ...
        to_value(&zeo_rt::dispatch::send_value(&meth, Symbol::intern("call"), &args, b)?)
    }
    ...
    fn rb_catch(tag: *const c_char, f: Option<BlockFunc>, arg: Value) -> Value {
        let t = if tag.is_null() {
            ...
            zeo_rt::builtins::string::str_value_in_enc(zeo_rt::encoding::UTF_8, &unsafe { cstr(tag) })
        };
        catch_with(t, f, arg)
    }
    ...
    fn rb_throw(tag: *const c_char, v: Value) -> () {
        let t = zeo_rt::builtins::string::str_value_in_enc(
    ...
    fn rb_eval_cmd_kw(cmd: Value, arg: Value, _kw: c_int) -> Value {
        ...
        match &c {
            ...
            _ => {
                ...
                to_value(&zeo_rt::dispatch::send_value(&c, Symbol::intern("call"), &args, None)?)
            }
        }
    }
    ...
    fn rb_provide(feature: *const c_char) -> () {
        ...
        zeo_rt::features::feature_loaded(0, &name, &name);
        Ok(())
    }
    ...
    fn rb_sourcefile() -> *const c_char {
        let file = zeo_rt::frames::current_location().map_or("", |(f, _)| f);
        Ok(super::symbol::cstr_for_owned(file))
    }

    fn rb_sourceline() -> c_int {
        Ok(zeo_rt::frames::current_location().map_or(0, |(_, l)| l as c_int))
    }

    fn rb_make_backtrace() -> Value {
        let lines: Vec<RubyValue> = zeo_rt::frames::capture_backtrace()
            .iter()
            .map(|l| zeo_rt::builtins::string::str_value_in_enc(zeo_rt::encoding::UTF_8, l))
            .collect();
        to_value(&RubyValue::Array(zeo_rt::value::collections::array_new(lines)))
    }
    ...
    fn rb_backtrace() -> () {
        for line in zeo_rt::frames::capture_backtrace() {
            let text = format!("\tfrom {line}\n");
            let _ = zeo_rt::builtins::io::write_bytes(
                &zeo_rt::builtins::io::current_stderr(),
                text.as_bytes(),
            );
        }
        Ok(())
    ...
    fn rb_make_exception(argc: c_int, argv: *const Value) -> Value {
        ...
        to_value(&match args.first() {
            ...
            Some(RubyValue::Str(s)) => {
                ...
                match zeo_rt::builtins::runtime_error!("{}", msg) {
                    Signal::Raise(e) => e,
    ...
    fn rb_backref_set(v: Value) -> () {
        zeo_rt::globals::global_set(0, "$~", unsafe { value_of(v) });
        Ok(())
    }

    fn rb_lastline_get() -> Value {
        to_value(&zeo_rt::globals::global_get(0, "$_"))
    }

    fn rb_lastline_set(v: Value) -> () {
        zeo_rt::globals::global_set(0, "$_", unsafe { value_of(v) });
        Ok(())
    }
    ...
    fn rb_enum_values_pack(argc: c_int, argv: *const Value) -> Value {
        ...
        match args.len() {
            0 => Ok(value::Q_NIL),
            1 => to_value(&args[0]),
            _ => to_value(&RubyValue::Array(zeo_rt::value::collections::array_new(args))),
        }
    }
...
fn thrown(tag: RubyValue, value: RubyValue) -> Signal {
    Signal::Throw(Box::new(zeo_rt::signal::Thrown { tag, value }))
}
...
fn is_provided(feature: &str) -> bool {
    let RubyValue::Array(loaded) = zeo_rt::globals::global_get(0, "$LOADED_FEATURES") else {
        return zeo_rt::features::has_feature(feature);
    };
    ...
    hit || zeo_rt::features::has_feature(feature)
}

fn eval_str(src: &str) -> Result<Value, Signal> {
    let text = zeo_rt::builtins::string::str_value_in_enc(zeo_rt::encoding::UTF_8, src);
    to_value(&zeo_rt::dispatch::send_value(
        &main_recv(),
        Symbol::intern("eval"),
        &[text],
...
fn require_named(feature: &str) -> Result<Value, Signal> {
    let name = zeo_rt::builtins::string::str_value_in_enc(zeo_rt::encoding::UTF_8, feature);
    to_value(&zeo_rt::dispatch::send_value(
        &main_recv(),
        Symbol::intern("require"),
        &[name],
...
fn frame_name() -> Id {
    match zeo_rt::frames::current_frame_method() {

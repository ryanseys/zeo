...
use zeo_rt::collections::{RArray, RHash};
...
unsafe fn as_ary(v: Value) -> Result<RArray, zeo_rt::Signal> {
    match unsafe { value_of(v) } {
        RubyValue::Array(a) => Ok(a),
        other => Err(zeo_rt::builtins::wrong_arg_type(&other, "Array")),
    }
}
...
///
/// `v` is an extension's own `VALUE`.
unsafe fn as_hash(v: Value) -> Result<RHash, zeo_rt::Signal> {
    match unsafe { value_of(v) } {
        RubyValue::Hash(h) => Ok(h),
        other => Err(zeo_rt::builtins::wrong_arg_type(&other, "Hash")),
    }
}

crate::cext_fn! {
    fn rb_ary_new() -> Value {
        to_value(&RubyValue::Array(zeo_rt::value::collections::array_new(Vec::new())))
    }
    ...
    fn rb_ary_push(v: Value, item: Value) -> Value {
        ...
        zeo_rt::builtins::array::array_push_checked(&a, item)?;
        Ok(v)
    }

    fn rb_ary_pop(v: Value) -> Value {
        let a = unsafe { as_ary(v)? };
        to_value(&zeo_rt::builtins::array::array_pop_checked(&a)?)
    }

    fn rb_ary_shift(v: Value) -> Value {
        let a = unsafe { as_ary(v)? };
        to_value(&zeo_rt::builtins::array::array_shift_checked(&a)?)
    }
    ...
    fn rb_hash_new() -> Value {
        to_value(&RubyValue::Hash(zeo_rt::value::collections::hash_new(Vec::new())))
    }
    ...
    fn rb_hash_aset(v: Value, key: Value, val: Value) -> Value {
        ...
        zeo_rt::value::collections::hash_set_checked(&h, key, val)?;
        Ok(v)
    }
...
pub(super) fn ary_ptr_len(v: Value) -> Result<(*const Value, c_long), zeo_rt::Signal> {
    let a = unsafe { as_ary(v)? };
...
crate::cext_fn! {
    ...
    fn rb_assoc_new(a: Value, b: Value) -> Value {
        ...
        to_value(&RubyValue::Array(zeo_rt::value::collections::array_new(pair)))
    }
    ...
    fn rb_ary_resurrect(v: Value) -> Value {
        ...
        to_value(&RubyValue::Array(zeo_rt::value::collections::array_new(copy)))
    }
    ...
    fn rb_hash_bulk_insert(n: c_long, pairs: *const Value, hash: Value) -> () {
        ...
        while i + 1 < n.max(0) {
            ...
            zeo_rt::value::collections::hash_set(&h, k, val);
            i += 2;
        }
        Ok(())
    }
    ...
    fn rb_hash_delete_if(v: Value) -> Value {
        ...
        for (k, val) in pairs {
            ...
            if super::convert::truthy(to_value(&verdict)?) {
                zeo_rt::value::collections::hash_delete(&h, &k);
            }
        }
        Ok(v)
    }
    ...
    fn rb_hash_update_by(
        ...
        for (k, new) in pairs {
            let old = zeo_rt::value::collections::hash_lookup(&d, &k);
            ...
            zeo_rt::value::collections::hash_set(&d, k, value);
        }
        Ok(dst)
    }
    ...
    fn rb_struct_initialize(recv: Value, values: Value) -> Value {
        ...
        let args = unsafe { splat(values) };
        zeo_rt::dispatch::send_value(&s, zeo_rt::Symbol::intern("initialize"), &args, None)?;
        Ok(recv)
    }
...
fn check_writable(a: &RArray) -> Result<(), zeo_rt::Signal> {
...
fn check_hash_writable(h: &RHash) -> Result<(), zeo_rt::Signal> {

...
pub(super) fn call(recv: &RubyValue, id: Id, args: &[RubyValue]) -> Result<RubyValue, Signal> {
    zeo_rt::dispatch::send_value(recv, symbol_of(id), args, None)
}
...
crate::cext_fn! {
    ...
    fn rb_jump_tag(_state: c_int) -> Value {
        Err(ERRINFO.with_borrow_mut(Option::take).map_or_else(
            || zeo_rt::builtins::runtime_error!("rb_jump_tag with no pending exception"),
            Signal::Raise,
        ))
    }
    ...
    fn rb_respond_to(v: Value, mid: Id) -> c_int {
        let v = unsafe { value_of(v) };
        Ok(c_int::from(zeo_rt::dispatch::responds_to_value(&v, symbol_of(mid), false)))
    }
...
fn rescued_by(exc: &RubyValue, classes: &[RubyValue]) -> bool {
    if classes.is_empty() {
        return zeo_rt::dispatch::is_a(exc.class_id(), zeo_abi::exc_id(4));
    }
...
fn raise_with(class: &RubyValue, msg: String) -> Signal {
    ...
    match zeo_rt::dispatch::send_value(class, Symbol::intern("exception"), &[text], None) {
        ...
        Err(_) => zeo_rt::builtins::type_error!(
            "exception class/object expected: {}",
            zeo_rt::dispatch::class_name(class.class_id()).unwrap_or("Object".into())
        ),
    }
}
...
pub(super) fn yield_to_block(args: &[RubyValue]) -> Result<RubyValue, Signal> {
    match current_block() {
        ...
        Some(other) => zeo_rt::dispatch::send_value(&other, Symbol::intern("call"), args, None),
        None => Err(zeo_rt::builtins::local_jump_error!(
...
fn value_is_a(v: &RubyValue, klass: &RubyValue) -> bool {
    match klass {
        RubyValue::Class(cid) => zeo_rt::dispatch::is_a(v.class_id(), *cid),
        _ => false,
    }
}

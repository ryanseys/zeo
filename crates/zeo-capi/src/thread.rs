...
fn nested(outer: &str, inner: &str) -> Result<RubyValue, Signal> {
    let RubyValue::Class(cid) = class_named(outer)? else {
        return Err(zeo_rt::builtins::type_error!("{outer} is not a class"));
    };
    zeo_rt::constants::const_get(cid.0, inner)
        .ok_or_else(|| zeo_rt::builtins::name_error!("uninitialized constant {outer}::{inner}"))
}
...
pub(super) unsafe fn block_proc(
    ...
    let addr = f as usize;
    zeo_rt::rproc::ProcBuilder::from_rust(
...
crate::cext_fn! {
    ...
    fn rb_thread_call_without_gvl(
        ...
        Ok(zeo_rt::gvl::without_gvl(|| unsafe { f(arg) }))
    }
    ...
    fn rb_thread_create(f: unsafe extern "C" fn(*mut c_void) -> Value, arg: *mut c_void) -> Value {
        ...
        .build();
        let thread = zeo_rt::dispatch::send_value(
            &class_named("Thread")?,
            Symbol::intern("new"),
            &[],
    ...
    fn rb_thread_interrupted(_t: Value) -> c_int {
        Ok(c_int::from(zeo_rt::gvl::interrupts_pending_anywhere()))
    }
...
fn sleep_for(sec: Option<RubyValue>) -> Result<(), Signal> {
    ...
    zeo_rt::dispatch::send_value(&main, Symbol::intern("sleep"), &args, None)?;
    Ok(())
}
...
fn new_fiber(
    ...
    let args: Vec<RubyValue> = storage
        .map(|s| {
            ...
            RubyValue::Hash(zeo_rt::value::collections::hash_new(vec![(key, s)]))
        })
        .into_iter()
        .collect();
    let fiber = zeo_rt::dispatch::send_value(
        &class_named("Fiber")?,
        Symbol::intern("new"),
        &args,
        Some(RubyValue::Proc(body)),
...
fn fiber_send(fiber: Value, meth: &str, argc: c_int, argv: *const Value) -> Result<Value, Signal> {
    ...
    if zeo_rt::dispatch::class_name(f.class_id()).as_deref() != Some("Fiber") {

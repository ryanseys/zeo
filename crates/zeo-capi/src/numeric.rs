...
impl Bad {
    fn signal(self, text: &str) -> Signal {
        match self {
            Bad::Radix(b) => {
                zeo_rt::builtins::arg_error!("invalid radix {b}")
            }
            Bad::Value(what) => {
                zeo_rt::builtins::arg_error!("invalid value for {what}(): \"{text}\"")
            }
        }
    }
}
...
fn parse_int_core(text: &str, base: c_int, badcheck: bool) -> Result<RubyValue, Bad> {
    ...
    Ok(zeo_rt::builtins::integer::int_value(if neg {
        -n
    } else {
        n
    }))
    ...
    let pair = match send(&yv, "coerce", std::slice::from_ref(&xv)) {
        ...
        _ => {
            ...
            return Err(zeo_rt::builtins::type_error!(
                ...
                zeo_rt::dispatch::class_name(yv.class_id()).unwrap_or("Object".into()),
                zeo_rt::dispatch::class_name(xv.class_id()).unwrap_or("Object".into())
            ));
        }
    };
...
fn both_orders(what: &str) -> Signal {
    zeo_rt::builtins::arg_error!("both MS{what} and LS{what} order flags are set")
}
...
crate::cext_fn! {
    ...
    fn rb_dbl2big(d: c_double) -> Value {
        if !d.is_finite() {
            return Err(zeo_rt::builtins::float_domain_error!("{}", RubyValue::Float(d).to_display_string()));
        }
        to_value(&zeo_rt::builtins::integer::int_value(
    ...
    fn rb_big_norm(v: Value) -> Value {
        to_value(&zeo_rt::builtins::integer::int_value(unsafe { as_big(v)? }))
    }
    ...
    fn rb_num_coerce_relop(x: Value, y: Value, op: super::symbol::Id) -> Value {
        ...
        if out == super::value::Q_NIL {
            let xv = unsafe { value_of(x) };
            let yv = unsafe { value_of(y) };
            return Err(zeo_rt::builtins::arg_error!("comparison of {} with {} failed",
                    zeo_rt::dispatch::class_name(xv.class_id()).unwrap_or("Object".into()),
    ...
    fn rb_integer_pack(
        ...
        if nails != 0 {
            return Err(zeo_rt::builtins::not_impl_error!("zeo's rb_integer_pack does not support nails"));
        }
    ...
    fn rb_integer_unpack(
        ...
        if nails != 0 {
            return Err(zeo_rt::builtins::not_impl_error!("zeo's rb_integer_unpack does not support nails"));
        }
        ...
        to_value(&zeo_rt::builtins::integer::int_value(n))
    }
}
...
fn int_to_str(n: &BigInt, base: c_int) -> Result<RubyValue, Signal> {
    if !(2..=36).contains(&base) {
        return Err(zeo_rt::builtins::arg_error!("invalid radix {base}"));
    }
    let v = zeo_rt::builtins::integer::int_value(n.clone());
...
fn complex_polar(abs: Value, arg: Value) -> Result<Value, Signal> {
    let cls = zeo_rt::constants::const_get(zeo_abi::OBJECT_CLASS.0, "Complex")
        .ok_or_else(|| zeo_rt::builtins::name_error!("uninitialized constant Complex"))?;
...
fn random_call(meth: &str, args: &[RubyValue]) -> Result<RubyValue, Signal> {
    let cls = zeo_rt::constants::const_get(zeo_abi::OBJECT_CLASS.0, "Random")
        .ok_or_else(|| zeo_rt::builtins::name_error!("uninitialized constant Random"))?;
    send(&cls, meth, args)
}

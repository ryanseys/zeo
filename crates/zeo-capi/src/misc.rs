...
fn enc_of(e: Encoding) -> EncodingId {
    match (e as usize).checked_sub(1) {
        ...
        None => zeo_rt::encoding::ASCII_8BIT,
    }
}
...
unsafe fn as_class(v: Value) -> Result<zeo_rt::dispatch::ClassId, Signal> {
    match unsafe { value_of(v) } {
        ...
        other => Err(zeo_rt::builtins::wrong_arg_type(&other, "Class")),
    }
}
...
crate::cext_fn! {
    ...
    fn rb_check_typeddata(obj: Value, expected: *const DataType) -> *mut c_void {
        ...
        let RubyValue::Object(o) = &v else {
            return Err(zeo_rt::builtins::wrong_arg_type(&v, "TypedData"));
        };
        ...
        Err(zeo_rt::builtins::wrong_arg_type(&v, "the expected TypedData"))
    }

    fn rb_utf8_encoding() -> Encoding {
        Ok(enc_token(zeo_rt::encoding::UTF_8))
    }

    fn rb_usascii_encoding() -> Encoding {
        Ok(enc_token(zeo_rt::encoding::US_ASCII))
    }

    fn rb_ascii8bit_encoding() -> Encoding {
        Ok(enc_token(zeo_rt::encoding::ASCII_8BIT))
    }

    fn rb_utf8_encindex() -> c_int {
        Ok(zeo_rt::encoding::UTF_8.0 as c_int)
    }

    fn rb_usascii_encindex() -> c_int {
        Ok(zeo_rt::encoding::US_ASCII.0 as c_int)
    }

    fn rb_ascii8bit_encindex() -> c_int {
        Ok(zeo_rt::encoding::ASCII_8BIT.0 as c_int)
    }
    ...
    fn rb_enc_get(obj: Value) -> Encoding {
        Ok(match unsafe { value_of(obj) } {
            ...
            _ => enc_token(zeo_rt::encoding::ASCII_8BIT),
        })
    }
    ...
    fn rb_enc_interned_str(p: *const c_char, len: c_long, e: Encoding) -> Value {
        let bytes = unsafe { super::string::borrow_bytes(p, len) };
        let s = RubyValue::Str(zeo_rt::string_from_bytes(bytes, enc_of(e)));
        s.freeze_value()?;
        to_value(&s)
    }
    ...
    fn rb_Array(v: Value) -> Value {
        ...
        to_value(&coerce(&v, "to_a").unwrap_or(RubyValue::Array(
            zeo_rt::value::collections::array_new(vec![v.clone()]),
        )))
    }
    ...
    fn rb_obj_classname(v: Value) -> *const c_char {
        let v = unsafe { value_of(v) };
        let name = zeo_rt::dispatch::class_name(v.class_id()).unwrap_or("Object".into());
        Ok(super::symbol::cstr_for_owned(&name))
    }

    fn rb_class2name(klass: Value) -> *const c_char {
        let cid = unsafe { as_class(klass)? };
        let name = zeo_rt::dispatch::class_name(cid).unwrap_or("Class".into());
        Ok(super::symbol::cstr_for_owned(&name))
    }
    ...
    fn rb_extend_object(target: Value, module: Value) -> Value {
        let t = unsafe { value_of(target) };
        let m = unsafe { value_of(module) };
        zeo_rt::runtime_meta::runtime_extend(&t, &m)?;
        Ok(target)
    }
    ...
    fn rb_str_resize(v: Value, len: c_long) -> Value {
        ...
        let RubyValue::Str(s) = &sv else {
            return Err(zeo_rt::builtins::wrong_arg_type(&sv, "String"));
        };
    ...
    fn rb_hash_foreach(
        ...
        let RubyValue::Hash(hh) = &hv else {
            return Err(zeo_rt::builtins::wrong_arg_type(&hv, "Hash"));
        };
    ...
    fn rb_big2ll(v: Value) -> i64 {
        match unsafe { value_of(v) } {
            ...
            other => Err(zeo_rt::builtins::wrong_arg_type(&other, "Integer")),
        }
    }
    ...
    fn rb_absint_size(v: Value, nlz_bits: *mut c_int) -> usize {
        let bits = match unsafe { value_of(v) } {
            ...
            other => return Err(zeo_rt::builtins::wrong_arg_type(&other, "Integer")),
        };
    ...
    fn rb_hash_lookup(h: Value, key: Value) -> Value {
        ...
        let RubyValue::Hash(h) = &h else {
            return Err(zeo_rt::builtins::wrong_arg_type(&h, "Hash"));
        };
        to_value(&zeo_rt::value::collections::hash_lookup(h, &key).unwrap_or(RubyValue::Nil))
    }
    ...
    fn rb_hash_lookup2(h: Value, key: Value, def: Value) -> Value {
        ...
        match zeo_rt::value::collections::hash_lookup(hh, &key) {
            Some(v) => to_value(&v),
            None => Ok(def),
        }
    }
...
fn coerce(v: &RubyValue, meth: &str) -> Result<RubyValue, Signal> {
    zeo_rt::dispatch::send_value(v, zeo_rt::Symbol::intern(meth), &[], None)
}

/// A conversion that answers nil instead -- the `rb_check_*_type` family.
fn check(v: &RubyValue, meth: &str) -> RubyValue {
    let sym = zeo_rt::Symbol::intern(meth);
    if !zeo_rt::dispatch::responds_to_value(v, sym, false) {
        return RubyValue::Nil;
    }
    zeo_rt::dispatch::send_value(v, sym, &[], None).unwrap_or(RubyValue::Nil)
}
...
pub unsafe extern "C" fn st_strncasecmp(a: *const c_char, b: *const c_char, n: usize) -> c_int {
    ...
    while i < n {
        ...
        let (x, y) = unsafe {
            (
                zeo_rt::c_char_u8(a.add(i).read()).to_ascii_lowercase(),
                zeo_rt::c_char_u8(b.add(i).read()).to_ascii_lowercase(),
            )
        };
        if x != y {
...
mod tests {
    ...
    fn an_encoding_token_round_trips_and_is_never_null() {
        ...
        assert_eq!(enc_of(std::ptr::null()), zeo_rt::encoding::ASCII_8BIT);
    }
    ...
    fn a_string_built_with_an_encoding_keeps_it() {
        ...
        assert_eq!(
            ...
            zeo_rt::encoding::UTF_8.0 as c_int
        );
    }

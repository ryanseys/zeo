...
fn last_errno() -> Signal {
    zeo_rt::builtins::file::raise_bare_errno(&std::io::Error::last_os_error())
}
...
crate::cext_fn! {
    ...
    fn rb_get_path(v: Value) -> Value {
        let val = unsafe { value_of(v) };
        to_value(&a_string(&zeo_rt::builtins::file::path_arg(&val, "to_path")?))
    }
    ...
    fn rb_find_file(path: Value) -> Value {
        let p = unsafe { value_of(path) };
        let name = zeo_rt::builtins::file::path_arg(&p, "to_path")?;
        to_value(&match zeo_rt::features::resolve_on_disk(&name, 0, true) {
    ...
    fn rb_find_file_ext(path: *mut Value, exts: *const *const c_char) -> c_int {
        ...
        let stem = zeo_rt::builtins::file::path_arg(&base, "to_path")?;
        let mut i = 0;
        loop {
            ...
            if let Some(found) = zeo_rt::features::resolve_on_disk(&name, 0, false) {
    ...
    fn rb_last_status_get() -> Value {
        to_value(&zeo_rt::globals::global_get(0, "$?"))
    }
    ...
    fn rb_last_status_set(status: c_int, pid: i32) -> () {
        ...
        let Some(st) = zeo_rt::constants::const_get(cid.0, "Status") else {
            return Ok(());
        };
        let built = send(
            &st,
    ...
    fn rb_exit(status: c_int) -> () {
        let main = zeo_rt::dispatch::main_object();
        zeo_rt::dispatch::send_value(
            &main,
            Symbol::intern("exit"),
    ...
    fn rb_env_clear() -> Value {
        let env = zeo_rt::constants::const_get(zeo_abi::OBJECT_CLASS.0, "ENV")
            .ok_or_else(|| zeo_rt::builtins::name_error!("uninitialized constant ENV"))?;
    ...
    fn rb_cloexec_open(path: *const c_char, flags: c_int, mode: u16) -> c_int {
        ...
        let Ok(c) = std::ffi::CString::new(p) else {
            return Err(zeo_rt::builtins::arg_error!("path contains a null byte"));
        };
    ...
    fn rb_io_get_io(v: Value) -> Value {
        ...
        match io_behind(&val)? {
            Some(io) => to_value(&io),
            None => Err(zeo_rt::builtins::type_error!(
                "no implicit conversion of {} into IO",
                zeo_rt::dispatch::class_name(val.class_id()).unwrap_or("Object".into())
            )),
        }
    }
    ...
    fn rb_io_mode(io: Value) -> c_int {
        ...
        Ok(zeo_rt::builtins::io::fmode_bits(&target) as c_int)
    }
    ...
    fn rb_io_taint_check(io: Value) -> Value {
        let target = unsafe { value_of(io) };
        let frozen = zeo_rt::dispatch::send_value(&target, Symbol::intern("frozen?"), &[], None)?;
        if matches!(frozen, RubyValue::Bool(true)) {
            return Err(zeo_rt::builtins::frozen_error!(
                "can't modify frozen {}",
                zeo_rt::dispatch::class_name(target.class_id()).unwrap_or("IO".into())
            ));
        }
        Ok(io)
    }
    ...
    fn rb_stat_new(st: *const libc::stat) -> Value {
        if st.is_null() {
            return Err(zeo_rt::builtins::arg_error!("rb_stat_new was given no stat"));
        }
        ...
        to_value(&zeo_rt::builtins::stat::stat_from_raw(unsafe { st.read() }))
    }
    ...
    fn rb_wait_for_single_fd(fd: c_int, events: c_int, tv: *mut libc::timeval) -> c_int {
        let timeout = if tv.is_null() {
            ...
            let ms = t.tv_sec * 1000 + zeo_rt::usec_i64(t.tv_usec) / 1000;
    ...
    fn rb_fdopen(fd: c_int, mode: *const c_char) -> *mut libc::FILE {
        ...
        let Ok(c) = std::ffi::CString::new(text) else {
            return Err(zeo_rt::builtins::arg_error!("mode contains a null byte"));
        };
...
fn io_behind(v: &RubyValue) -> Result<Option<RubyValue>, Signal> {
    if zeo_rt::builtins::io::as_rio(v).is_some() {
        return Ok(Some(v.clone()));
    }
    let to_io = Symbol::intern("to_io");
    if !zeo_rt::dispatch::responds_to_value(v, to_io, true) {
        return Ok(None);
    }
    let out = zeo_rt::dispatch::send_value(v, to_io, &[], None)?;
    if zeo_rt::builtins::io::as_rio(&out).is_none() {
...
fn wait_retry(fd: c_int, event: i16) -> Result<c_int, Signal> {
    if fd < 0 {
        return Err(zeo_rt::builtins::io_error!("closed stream"));
    }
...
crate::cext_fn! {
    ...
    fn rb_zeo_rfile(obj: Value) -> *mut mri::RFile {
        ...
        let Some(rio) = zeo_rt::builtins::io::as_rio(&target) else {
            return Err(wrong_arg_type(&target, "IO"));
        };
        ...
        unsafe {
            ...
            (*block).io.mode = zeo_rt::builtins::io::fmode_bits(&target) as mri::rb_io_mode;
...
fn modestr_fmode(text: &str) -> Result<c_int, Signal> {
    let bad = || zeo_rt::builtins::arg_error!("invalid access mode {text}");
...
unsafe fn io_of(fptr: *mut mri::rb_io) -> Result<RubyValue, Signal> {
    if fptr.is_null() {
        return Err(zeo_rt::builtins::io_error!("uninitialized stream"));
    }
...
crate::cext_fn! {
    ...
    fn rb_io_read_pending(fptr: *mut mri::rb_io) -> c_int {
        ...
        Ok(zeo_rt::builtins::io::has_buffered_bytes(&target) as c_int)
    }
    ...
    fn rb_io_read_check(fptr: *mut mri::rb_io) -> () {
        let target = unsafe { io_of(fptr)? };
        if zeo_rt::builtins::io::has_buffered_bytes(&target) {
            return Ok(());
        }
    ...
    fn rb_io_bufwrite(io: Value, buf: *const std::ffi::c_void, size: usize) -> isize {
        ...
        let s = RubyValue::Str(zeo_rt::string_from_bytes(bytes, zeo_rt::encoding::ASCII_8BIT));
    ...
    fn rb_io_stdio_file(fptr: *mut mri::rb_io) -> *mut libc::FILE {
        ...
        let target = unsafe { io_of(fptr)? };
        let fmode = zeo_rt::builtins::io::fmode_bits(&target);
    ...
    fn rb_io_open_descriptor(
        ...
        if !matches!(path_v, RubyValue::Nil) {
            args.push(RubyValue::Hash(zeo_rt::value::collections::hash_new(vec![(
...
fn io_check(fptr: *mut mri::rb_io, want: Want) -> Result<(), Signal> {
    ...
    let mode = zeo_rt::builtins::io::fmode_bits(&target);
    match want {
        Want::Readable if mode & READABLE == 0 => {
            Err(zeo_rt::builtins::io_error!("not opened for reading"))
        }
        Want::Writable if mode & WRITABLE == 0 => {
            Err(zeo_rt::builtins::io_error!("not opened for writing"))
        }
        _ => Ok(()),
    }
}
...
fn write_error(bytes: Vec<u8>) -> Result<(), Signal> {
    let _ = zeo_rt::builtins::io::write_bytes(&zeo_rt::builtins::io::current_stderr(), &bytes);
    Ok(())
}

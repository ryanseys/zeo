...
use zeo_rt::{RProc, RubyValue, Signal, Symbol};
...
fn bad_argc(argc: c_int) -> Signal {
    zeo_rt::builtins::arg_error!("arity out of range: {argc} for -2..{MAX_FIXED_ARITY}")
}
...
pub unsafe fn method_proc(f: MethodPtr, argc: c_int) -> Result<RProc, Signal> {
    ...
    let addr = f as usize;
    Ok(zeo_rt::rproc::ProcBuilder::from_rust(
        move |recv, args, block| {
            ...
            if argc >= 0 && raw.len() != argc as usize {
                return Err(zeo_rt::dispatch::wrong_arity(raw.len(), &argc.to_string()));
            }

            let (fixed, packed);
            let (argc, args) = if argc == ARGC_ARRAY {
                packed = [to_value(&RubyValue::Array(
                    zeo_rt::value::collections::array_new(args.to_vec()),
                ))?];
                (1, &packed[..])
            } else {
                fixed = raw;
...
/// plain Ruby one.
static ALLOC_FUNCS: parking_lot::Mutex<Option<zeo_rt::FMap<u32, usize>>> =
    parking_lot::Mutex::new(None);

/// Run a class's C allocator, if it has one. `dispatch::allocate_of` asks,
/// through the runtime's hook, which has already paid the one relaxed load
/// (`capi_hooks::any_alloc_func`) that keeps a program with no extension
/// off this map.
///
...
pub fn c_allocate(owner: ClassId) -> Option<RubyValue> {
    ...
    if addr == ALLOC_UNDEF {
        zeo_rt::signal::set_pending(zeo_rt::builtins::type_error!(
            "allocator undefined for {}",
            zeo_rt::dispatch::class_name(owner).unwrap_or_else(|| format!("#<Class:{}>", owner.0))
        ));
        return None;
    }
    let scope = Scope::enter();
...
fn remember_alloc_func(owner: ClassId, addr: usize) {
    ALLOC_FUNCS
        .lock()
        .get_or_insert_with(zeo_rt::FMap::default)
        .insert(owner.0, addr);
    zeo_rt::capi_hooks::note_alloc_func();
}
...
fn resolved_alloc_func(owner: ClassId) -> Option<usize> {
    ...
    for &anc in zeo_rt::dispatch::ancestors_of_value(owner).iter() {
        if anc == owner || zeo_rt::dispatch::class_is_module(anc).unwrap_or(true) {
            continue;
        }
...
/// TypeError.
pub fn has_alloc_func(owner: ClassId) -> bool {
...
unsafe fn as_class(v: Value) -> Result<ClassId, Signal> {
    match unsafe { value_of(v) } {
        ...
        other => Err(zeo_rt::builtins::wrong_arg_type(&other, "Class/Module")),
    }
}

fn install(owner: ClassId, name: Symbol, body: RProc) -> Result<(), Signal> {
    zeo_rt::runtime_meta::runtime_define_method(owner, name, body)?;
    Ok(())
}
...
fn reopen_target(outer: ClassId, name: &str) -> Option<RubyValue> {
    if let Some(existing @ RubyValue::Class(_)) = zeo_rt::constants::const_get_own(outer.0, name) {
        return Some(existing);
    }
    let full = qualified(outer, name);
    let cid = zeo_rt::dispatch::registered_class_id_by_name(&full)?;
    ...
    zeo_rt::constants::reveal_class(cid.0);
    Some(RubyValue::Class(cid))
}

fn qualified(outer: ClassId, name: &str) -> String {
    match zeo_rt::dispatch::class_name(outer) {
...
crate::cext_fn! {
    ...
    fn rb_define_protected_method(klass: Value, name: *const c_char, f: MethodPtr, argc: c_int) -> () {
        ...
        zeo_rt::runtime_meta::runtime_set_visibility(
            owner,
            &[RubyValue::Symbol(sym)],
            zeo_rt::dispatch::MethodVisibility::Protected,
        )?;
        Ok(())
    }
    ...
    fn rb_define_singleton_method(obj: Value, name: *const c_char, f: MethodPtr, argc: c_int) -> () {
        ...
        let sym = Symbol::intern(&unsafe { cname(name) });
        zeo_rt::runtime_meta::runtime_define_singleton_method(
            &recv,
            sym,
    ...
    fn rb_define_alias(klass: Value, new: *const c_char, old: *const c_char) -> () {
        ...
        zeo_rt::runtime_meta::runtime_alias_method(owner, new, old)?;
        Ok(())
    }
    ...
    fn rb_ivar_get(obj: Value, id: Id) -> Value {
        ...
        to_value(&zeo_rt::dispatch::instance_variable_get(&recv, &name)?)
    }

    fn rb_ivar_set(obj: Value, id: Id, val: Value) -> Value {
        let recv = unsafe { value_of(obj) };
        let name = RubyValue::Symbol(symbol_of(id));
        zeo_rt::dispatch::instance_variable_set(&recv, &name, unsafe { value_of(val) })?;
        Ok(val)
    }
}
...
fn new_module(outer: ClassId, name: &str) -> Result<Value, Signal> {
    ...
    let m = zeo_rt::runtime_meta::runtime_module_new(None)?;
    if let RubyValue::Class(cid) = &m {
        let full = qualified(outer, name);
        zeo_rt::runtime_meta::name_runtime_class_if_anonymous(*cid, &full);
        zeo_rt::constants::const_set(outer.0, name, m.clone());
    }
    to_value(&m)
}

...
fn gc_module() -> Result<RubyValue, Signal> {
    zeo_rt::constants::const_get(zeo_abi::OBJECT_CLASS.0, "GC")
        .ok_or_else(|| zeo_rt::builtins::name_error!("uninitialized constant GC"))
}
...
crate::cext_fn! {
    ...
    fn rb_alloc_tmp_buffer_with_count(store: *mut Value, size: usize, count: usize) -> *mut c_void {
        let total = size.checked_mul(count).ok_or_else(|| {
            zeo_rt::builtins::no_memory_error!("buffer size overflow")
        })?;
    ...
    fn rb_copy_generic_ivar(dst: Value, src: Value) -> () {
        ...
        for name in names {
            let v = zeo_rt::dispatch::instance_variable_get(&s, &name)?;
            zeo_rt::dispatch::instance_variable_set(&d, &name, v)?;
        }
        Ok(())
    }
...
fn tmp_buffer(store: *mut Value, len: usize) -> Result<*mut c_void, Signal> {
    ...
    let holder = to_value(&zeo_rt::builtins::string::str_value_in_enc(

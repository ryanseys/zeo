...
crate::cext_fn! {
    ...
    fn rb_uint2inum(n: usize) -> Value {
        to_value(&zeo_rt::builtins::integer::int_value(
    ...
    fn rb_must_asciicompat(v: Value) -> () {
        ...
        if !enc.ascii_compatible() {
            return Err(zeo_rt::dispatch::raise_error(
    ...
    fn rb_uv_to_utf8(buf: *mut c_char, uv: usize) -> c_int {
        let Some(ch) = u32::try_from(uv).ok().and_then(char::from_u32) else {
            return Err(zeo_rt::builtins::range_error!("pack(U): value out of range: {uv}"));
        };
        let mut tmp = [0u8; 4];
    ...
    fn rb_get_kwargs(
        ...
        let h = match unsafe { value_of(hash) } {
            RubyValue::Hash(h) => h,
            RubyValue::Nil => zeo_rt::value::collections::hash_new(Vec::new()),
        ...
        for i in 0..named {
            ...
            let found = zeo_rt::value::collections::hash_lookup(&h, &key);
            let slot = match (found, i < required.max(0) as usize) {
                (Some(v), _) => to_value(&v)?,
                // A missing REQUIRED key is an error naming it.
                (None, true) => {
                    return Err(zeo_rt::builtins::arg_error!("missing keyword: :{}", sym.name_str()));
                }
        ...
        let rest: Vec<(RubyValue, RubyValue)> = h
            ...
            .collect();
        if rest_wanted {
            let leftover = RubyValue::Hash(zeo_rt::value::collections::hash_new(rest));
            if !values.is_null() {
        ...
        if let Some((k, _)) = rest.first() {
            return Err(zeo_rt::builtins::arg_error!("unknown keyword: {}", k.to_display_string()));
        }
        Ok(named as c_int)
    }
    ...
    fn rb_extract_keywords(slot: *mut Value) -> Value {
        ...
        let leftover = if rest.is_empty() {
            ...
            to_value(&RubyValue::Hash(zeo_rt::value::collections::hash_new(rest)))?
        };
        unsafe { slot.write(leftover) };
        to_value(&RubyValue::Hash(zeo_rt::value::collections::hash_new(syms)))
    }
    ...
    fn rb_get_values_at(
        ...
        to_value(&RubyValue::Array(zeo_rt::value::collections::array_new(out)))
    }
    ...
    fn rb_class_descendants(klass: Value) -> Value {
        ...
        let mut out: Vec<RubyValue> = Vec::new();
        for name in zeo_rt::dispatch::nested_class_names(zeo_abi::OBJECT_CLASS) {
            if let Some(v @ RubyValue::Class(other)) =
                zeo_rt::constants::const_get(zeo_abi::OBJECT_CLASS.0, &name)
                && other != cid
                && zeo_rt::dispatch::is_a(other, cid)
            {
                out.push(v);
            }
        }
    ...
    fn rb_out_of_int(n: c_long) -> () {
        Err(zeo_rt::builtins::range_error!("integer {n} too big to convert to `int'"))
    }
    ...
    fn rb_scan_args_bad_format(fmt: *const c_char) -> () {
        Err(zeo_rt::dispatch::raise_error(
            "FatalError",
    ...
    fn rb_varargs_bad_length(passed: c_int, expected: c_int) -> c_int {
        Err(zeo_rt::builtins::arg_error!("wrong number of arguments ({passed} for {expected})"))
    }
    ...
    fn rb_arithmetic_sequence_extract(v: Value, out: *mut ArithSeq) -> c_int {
        ...
        let name = zeo_rt::dispatch::class_name(seq.class_id()).unwrap_or_default();
    ...
    fn rb_thread_fd_select(
        ...
        if out < 0 {
            return Err(zeo_rt::builtins::file::raise_bare_errno(
    ...
    fn ruby_malloc_size_overflow(count: usize, size: usize) -> () {
        Err(zeo_rt::builtins::no_memory_error!("malloc: possible integer overflow ({count} * {size})"))
    }

    fn ruby_malloc_add_size_overflow(a: usize, b: usize) -> () {
        Err(zeo_rt::builtins::no_memory_error!("malloc: possible integer overflow ({a} + {b})"))
    }
...
fn cmperr(a: Value, b: Value) -> Signal {
    ...
    zeo_rt::builtins::arg_error!(
        "comparison of {} with {} failed",
        zeo_rt::dispatch::class_name(x.class_id()).unwrap_or("Object".into()),
        y.to_display_string()
    )
}
...
fn outside_str(p: *const c_char, len: c_long) -> Result<Value, Signal> {
    let bytes = unsafe { super::string::borrow_bytes(p, len) };
    to_value(&RubyValue::Str(zeo_rt::string_from_bytes(
        bytes,
...
fn mirror(cell: &'static std::sync::atomic::AtomicUsize, name: &str) -> *mut Value {
    let held = zeo_rt::globals::global_get(0, name);
...
fn select_on(fd: c_int, writable: bool, timeout: Option<f64>) -> Result<c_int, Signal> {
    ...
    let list = RubyValue::Array(zeo_rt::value::collections::array_new(vec![target]));

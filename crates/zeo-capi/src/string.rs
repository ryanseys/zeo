...
fn publish_pin(key: usize, new_len: usize) -> Result<bool, zeo_rt::Signal> {
    PINS.with_borrow_mut(|pins| {
        let Some((_, pin)) = pins.iter_mut().find(|(k, _)| *k == key) else {
            return Ok(false);
        };
...
unsafe fn as_str(v: Value) -> Result<RStr, zeo_rt::Signal> {
    match unsafe { value_of(v) } {
        RubyValue::Str(s) => Ok(s),
        other => Err(zeo_rt::builtins::wrong_arg_type(&other, "String")),
    }
}
...
fn new_str(bytes: Vec<u8>, enc: zeo_rt::encoding::EncodingId) -> RubyValue {
    RubyValue::Str(zeo_rt::string_from_bytes(bytes, enc))
}
...
crate::cext_fn! {
    ...
    fn rb_str_new(p: *const c_char, len: c_long) -> Value {
        ...
        to_value(&new_str(bytes, zeo_rt::encoding::ASCII_8BIT))
    }
    ...
    fn rb_utf8_str_new(p: *const c_char, len: c_long) -> Value {
        let bytes = unsafe { borrow_bytes(p, len) };
        to_value(&new_str(bytes, zeo_rt::encoding::UTF_8))
    }
    ...
    fn rb_usascii_str_new(p: *const c_char, len: c_long) -> Value {
        let bytes = unsafe { borrow_bytes(p, len) };
        to_value(&new_str(bytes, zeo_rt::encoding::US_ASCII))
    }
    ...
    fn rb_str_buf_new(capa: c_long) -> Value {
        let s = new_str(Vec::new(), zeo_rt::encoding::ASCII_8BIT);
...
fn code_unit_width(enc: zeo_rt::encoding::EncodingId) -> usize {
    match enc.spec().kind {
        zeo_rt::encoding::EncKind::Utf16 { .. } => 2,
        zeo_rt::encoding::EncKind::Utf32 { .. } => 4,
...
crate::cext_fn! {
    ...
    fn rb_str_tmp_new(len: c_long) -> Value {
        to_value(&new_str(vec![0; len.max(0) as usize], zeo_rt::encoding::ASCII_8BIT))
    }
    ...
    fn rb_interned_str(p: *const c_char, len: c_long) -> Value {
        let bytes = unsafe { borrow_bytes(p, len) };
        interned(bytes, zeo_rt::encoding::UTF_8)
    }
    ...
    fn rb_str_append(dst: Value, src: Value) -> Value {
        ...
        if !matches!(s, RubyValue::Str(_)) {
            return Err(zeo_rt::builtins::wrong_arg_type(&s, "String"));
        }
    ...
    fn rb_str_hash(v: Value) -> usize {
        let s = unsafe { as_str(v)? };
        let g = s.lock();
        Ok(crate::st::bytes_hash_of(g.bytes()))
    }
    ...
    fn rb_str_split(v: Value, sep: *const c_char) -> Value {
        let sv = unsafe { value_of(v) };
        let sep = new_str(unsafe { borrow_bytes(sep, -1) }, zeo_rt::encoding::UTF_8);
    ...
    fn rb_str_format(argc: c_int, argv: *const Value, fmt: Value) -> Value {
        ...
        let arr = RubyValue::Array(zeo_rt::value::collections::array_new(args));
    ...
    fn rb_string_value_cstr(slot: *mut Value) -> *mut c_char {
        ...
        if s.lock().bytes().contains(&0) {
            return Err(zeo_rt::builtins::arg_error!("string contains null byte"));
        }
        Ok(pin_bytes(&s))
    }
}
...
fn check_writable(s: &RStr) -> Result<(), zeo_rt::Signal> {
...
fn copy_of(v: Value) -> Result<Value, zeo_rt::Signal> {
    let s = unsafe { as_str(v)? };
    let g = s.lock();
...
fn interned(bytes: Vec<u8>, enc: zeo_rt::encoding::EncodingId) -> Result<Value, zeo_rt::Signal> {
...
fn string_value_in(slot: *mut Value) -> Result<Value, zeo_rt::Signal> {
    if slot.is_null() {
    ...
    if !matches!(out, RubyValue::Str(_)) {
        return Err(zeo_rt::builtins::wrong_arg_type(&v, "String"));
    }
...
pub(super) fn str_ptr_len(v: Value) -> Result<(*mut c_char, c_long), zeo_rt::Signal> {
    let s = unsafe { as_str(v) }?;
    let ptr = pin_bytes(&s);
    let key = std::sync::Arc::as_ptr(&s) as *const () as usize;
    let len = PINS
...
mod tests {
    ...
    fn a_new_string_carries_the_bytes_and_the_encoding() {
        ...
        match &v {
            RubyValue::Str(s) => assert_eq!(s.lock().encoding(), zeo_rt::encoding::UTF_8),
            _ => panic!("not a String"),
        }
    }

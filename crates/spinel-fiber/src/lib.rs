//! spinel-fiber: the "current yielder" shim that lets `Fiber.yield` suspend
...
//! `unsafe` code** (`spinelc`/`spinel-rt` keep `#![forbid(unsafe_code)]`) --
...
//!    panics cleanly instead of reinterpreting memory. (`spinel-rt` only
...
//! spinel-generated code never suspends from a destructor (Ruby-level

...
//!
//! A gemspec is Ruby source, but the one spinel needs to read is not
...
//!    `Gem::StubSpecification` exists: everything spinel consumes is available
//!    without parsing the body at all.
//! 2. **A static prism parse of the body**, for gemspecs with no stub header --
//!    including the ones spinel itself ships under `gems/`, which are
...
//! RHS shapes spinel never reads would reject perfectly valid gemspecs for no
...
pub(super) struct GemSpec {
    pub name: String,
    /// `None` for a hand-written manifest that omits it. spinel does not
...
/// Walk one statement list, recording the attributes spinel consumes.
///
...
fn visit_statements(node: &ruby_prism::Node<'_>, spec: &mut GemSpec) -> PResult<()> {
    ...
    for stmt in stmts.body().iter() {
        ...
        let Some(attr) = method.strip_suffix('=') else {
            // `s.add_runtime_dependency(...)` and friends -- spinel resolves no
...
fn want_string_array(node: &ruby_prism::Node<'_>, attr: &str) -> PResult<Vec<String>> {
    ...
    let array = node.as_array_node().ok_or_else(|| {
        format!("`s.{attr}` must be a string or an array of string literals (spinel parses gemspecs statically, without evaluating them)")
    })?;
    array
        .elements()
        .iter()

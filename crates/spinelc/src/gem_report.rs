...
//! how spinel actually satisfied it -- and, where spinel's answer is NOT the
...
//! spinel's `json` behaves *almost* like the gem until an edge case where it
...
/// external gem store (Phase 3), where a native gem spinel has no ext for is a
...
pub enum SatisfiedBy {
    ...
    /// A spinel-bundled gem under `gems/<name>/` (`require "optparse"`).
...
pub const SUBSTITUTE_SLUG: &str = "spinel-builtin-substitute";

/// If spinel's implementation of `name` is NOT the upstream gem/extension,
...
pub fn substitution_note(name: &str) -> Option<&'static str> {
    match name {
        ...
        "strscan" => Some("a spinel reimplementation of StringScanner"),
        "stringio" => Some("a spinel reimplementation of StringIO"),
        "date" => Some("a spinel reimplementation of Date/DateTime"),
        "socket" => Some("a partial spinel reimplementation"),
        "base64" => Some("a spinel reimplementation of Base64"),
        "cgi" | "cgi/escape" => Some("a spinel reimplementation of CGI escaping"),
        _ => None,
    }
}

/// Write `target/spinel-gems.json` (or wherever the CLI pointed): one object
    ...
    for r in records {
        if let Some(note) = substitution_note(&r.name) {
            if seen.insert(r.name.as_str()) {
                eprintln!(
                    "spinelc: warning: '{}' is satisfied by spinel's built-in implementation \

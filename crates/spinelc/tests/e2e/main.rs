...
//! Compiles each snippet with `spinelc` in-process, builds and runs the
...
//! compile errors. `examples/*.rb` + `.expected` remain as a smaller "real CLI
//! + real `ruby`-oracle" smoke suite driven by `cargo run -p xtask -- test`;

...
pub fn run_ruby_packages(
    ...
    zeo::build::build_binary(
        rust_source,
        &bin,
        zeo::build::Linkage::Dynamic,
        zeo::build::Profile::Debug,
        runtime,
    )
        .unwrap_or_else(|e| {
...
pub fn run_ruby_configured(source: &str, env: &[(&str, &str)], args: &[&str]) -> RunResult {
    ...
    zeo::build::ensure_runtime_built(zeo::build::Profile::Debug, runtime)
        .expect("building zeo-rt for the e2e harness");
    zeo::build::build_binary(rust_source, &bin, zeo::build::Linkage::Dynamic, zeo::build::Profile::Debug, runtime).unwrap_or_else(|e| {
        panic!("build_binary failed: {e}\n--- generated Rust ---\n{rust_source}")
    });

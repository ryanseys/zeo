//! A host merging a package: what the merged program answers, and what
//! each refusal names.
...
fn a_host_using_a_packaged_refinement_module_refuses() {
    ...
    assert!(
        !out.status.success(),
        "a host `using` of a package module must refuse"
    );
    let err = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn a_package_refining_an_unknown_target_refuses_instead_of_dropping() {
    ...
    assert!(
        !out.status.success(),
        "an unresolvable refine target must refuse"
    );
    let err = String::from_utf8_lossy(&out.stderr);
    assert!(

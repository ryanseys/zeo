...
pub(super) fn try_collection_dispatch(
    ...
    // A plain hoisted local is BORROWED rather than cloned. Every row below
    // reads the receiver through `as_array_ref`/`as_hash_ref`/`as_str_ref`
    // -- borrows of the handle inside the value -- yet the receiver arrived
    // as `Clone::clone(&a)`, so `a << x` paid an `Arc` bump and a drop per
    // call for a handle it only ever reads through. `borrowable_operand`'s
    // own soundness argument applies verbatim: these arms only READ through
    // the borrow, and a local any closure could reassign would have forced
    // `Captured` (cell) storage and not qualify.
    let borrowed;

...
// The fusion fast-path predicates live in `analyze::fastpath` (they are pure
...
pub fn emit_call(
        ...
        && let Some(target) = cx.resolve_class(&target_path)
    {
        ...
        if name == "method"
            ...
            && matches!(&cx.compiler.hir[args[0]], HirNode::SymbolLit(sym)
                if crate::analyze::class_query::class_method_defined_only_later(
                    cx.compiler, target, sym, recv_id))
        {

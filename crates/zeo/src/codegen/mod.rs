...
/// Runs codegen into `sink`, turning the unsupported-construct record into a
/// clean `CompileError` -- the shared front half of both renderers below.
///
/// The record is only readable once emission has FINISHED, so a streaming
/// sink has already written a partial program by the time this reports. That
/// is why `compile_to_file` deletes its file on error: half a program parses
/// as a whole one, and the next tool along would blame rustc.
fn codegen_into(
    analyzed: &Analyzed,
    sink: &mut ItemSink<'_>,
) -> Result<(), crate::diagnostics::CompileError> {
    take_unsupported();
    let io = codegen(analyzed, sink);
...
/// The whole program as one `TokenStream`, for the renderer that needs it
/// assembled (`--dump=rust`, and the `ZEO_VALIDATE` net).
fn codegen_to_tokens(analyzed: &Analyzed) -> Result<TokenStream, crate::diagnostics::CompileError> {
    let mut sink = ItemSink::Collect(TokenStream::new());
    codegen_into(analyzed, &mut sink)?;
    Ok(sink.into_tokens())
}
...
/// [`codegen_to_string`], streamed -- and the reason `ItemSink` exists.
///
/// Each top-level item is rendered and written as it is produced, so neither
/// the program's tokens nor its text is ever held whole. Under
/// `ZEO_VALIDATE` the collecting path runs instead, because `syn` has to see
/// the program as a unit; that is a debugging switch, and it pays for what it
/// asks for.
pub fn codegen_to_writer<W: std::io::Write>(
    ...
    };
    {
        ...
        counting
            .write_all(b"// Generated by zeo. Do not edit by hand.\n\n")
            .map_err(|e| {
                crate::diagnostics::CompileError::codegen(format!(
                    "writing the generated Rust: {e}"
                ))
            })?;
    }
    if crate::debug_flags::debug(crate::debug_flags::DebugFlag::Validate) {
        ...
        write_line_broken(tokens, &mut counting).map_err(|e| {
            crate::diagnostics::CompileError::codegen(format!("writing the generated Rust: {e}"))
        })?;
    } else {
        codegen_into(analyzed, &mut ItemSink::Writer(&mut counting))?;
    }
    {
        use std::io::Write as _;
        counting.flush().map_err(|e| {
            crate::diagnostics::CompileError::codegen(format!("writing the generated Rust: {e}"))
        })?;
    }
    if crate::timings_enabled() {
        eprintln!("zeo-timings: emit={}ms", t_emit.elapsed().as_millis());
    }
    Ok(counting.stats)
}
...
/// The file's inner attributes and its allocator, which must lead.
fn prelude_attrs() -> TokenStream {
    quote! {
        ...
        // A rustc-backed binary swaps in mimalloc: Ruby workloads are
        // allocation-heavy, and a self-contained binary owns every
        // allocation. The backend arms the cfg on every such build now that
        // they are all self-contained; the dynamic-linkage variant this once
        // had to exclude went with the bin cache it existed for (declaring
        // the allocator in either image of an exe/dylib pair aborts in
        // libmalloc at bootstrap).
...
/// Where each of the program's top-level items goes as it is produced.
///
/// The build path WRITES and drops, so only one item's tokens are alive at a
/// time. Holding the whole program as tokens is the compiler's largest single
/// allocation -- roughly ten to fifteen bytes of memory per byte of final
/// text -- and nothing on that path ever needs it assembled.
///
/// `--dump=rust` still collects: `syn` has to parse the program as a unit, so
/// that renderer pays for the copy it actually uses.
///
/// Rust top-level items are order-independent, which is what makes this a
/// choice of sink rather than a rewrite. The one ordering rule is that the
/// inner attributes go first, so [`Self::open`] pushes them.
pub(crate) enum ItemSink<'a> {
    Writer(&'a mut dyn std::io::Write),
    Collect(TokenStream),
}

impl ItemSink<'_> {
    fn push(&mut self, item: TokenStream) -> std::io::Result<()> {
        match self {
            ItemSink::Writer(w) => write_line_broken(item, w),
            ItemSink::Collect(acc) => {
                acc.extend(item);
                Ok(())
            }
        }
...
fn codegen(analyzed: &Analyzed, sink: &mut ItemSink<'_>) -> std::io::Result<()> {
    ...
    // The attributes come first because inner attributes must; everything
    // after them is order-free, which is what lets the four biggest item
    // groups below go straight out instead of being collected.
    sink.push(prelude_attrs())?;
    if let Some(container) = shared.container() {
        sink.push(container.clone())?;
    }

    // Streamed, not collected. These four groups are the bulk of a generated
    // program -- for `uri` the shared-body container alone is a quarter of it
    // -- and each item is finished the moment it is produced. Collecting them
    // meant the whole program existed as tokens before a byte was written.
    //
    // Emission order still matters for a different reason: every body feeds
    ...
    for (idx, class) in compiler.classes.iter().enumerate() {
        // `Object` (idx 0) rides the same machinery since top-level `def`
        // support: its methods emit into `__bm_Object` and register as value
        // methods, dispatched on the `main` object at top-level call sites.
    ...
    // The remaining item groups, out one at a time like the four above. These
    // are produced by the middle of this function rather than at its head, so
    // they are streamed here instead of where they are built.
    for group in [
        um_containers,
        redef_containers,
        exc_containers,

...
//! verbatim. zeo never resolves, fetches, or version-solves; it reads the
...
//! preferred, because a precompiled platform gem ships a `.bundle` zeo can
...
mod tests {
    ...
    ///
    /// The fixture is the one gem store the e2e suite (tests/e2e/ffi.rs)
    /// also compiles against; a second copy here would drift from it. It
    /// lives under /tests, which the packaging whitelist excludes, so the

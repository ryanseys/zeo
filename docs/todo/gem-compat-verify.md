...
"spinel resolved the gem's `lib/` and *would attempt* to compile it" — a static
...
1. Classify as today (reuse `spinelc::gem_compat` / `gem_compat_installed`).
2. For every `pure-ruby` gem, actually compile a probe:
   ...
   - run the `spinelc` binary on a `require "<name>"` probe with `--gem-path
   ...
   - map the outcome: exit 0 → `compiles`; a clean rejection or a compiler panic
     ...
     and share it).
3. Parallelize like `stdlib-status` (worker pool over a queue; `spinelc` built
...
- A gem's probe pulls its whole `require` graph, so a failure may be a
  transitive dep (a native gem, or a stdlib file spinel lacks), not the gem
...
The static classification already answers "which gems is spinel even in a

...
| io/console | `io/console` | `ext-io-console` | **done** | the terminal modes (`raw`/`cooked`/`echo=`/`getch`/`getpass` over `termios(3)`), `winsize` over `ioctl`, the cursor escapes, and `IO::Console::Mode`; the rows live on `IO` itself, gated on the require |

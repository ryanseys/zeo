...
# receiver index read), where no caller passes a non-nil `other`. What the
# body DOES with the parameter says what it holds: a String-keyed hash read
# means the parameter is a hash, whatever its `nil` default suggests.
#
# Both guards -- `return if other.nil?` and `if !v.nil?` -- have to stay
# live, which they cannot if the parameter is reasoned about as Integer 0,
# whose `.nil?` is false.

...
log = File.open(File.join(ZTMP, "round-trip.log"), "wb")
saved = $stdout.dup
...
log.close
saved.close
puts "the log holds: #{File.read(File.join(ZTMP, "round-trip.log")).split("\n").inspect}"
File.unlink(File.join(ZTMP, "round-trip.log"))

# A dup of a closed stream still raises, and a File dup still reads the file.
File.write(File.join(ZTMP, "payload.txt"), "abc")
f = File.open(File.join(ZTMP, "payload.txt"))
g = f.dup
puts "a File dup reads:   #{g.read}"
f.close
g.close
File.unlink(File.join(ZTMP, "payload.txt"))
__END__
a fresh descriptor: true
a plain IO:         IO
back on the console
the log holds: ["into the log"]
a File dup reads:   abc

...
| count | reason |
|------:|--------|
| 163 | `require` is only supported as a top-level statement with a single string-literal argument (spike sc… |
| 58 | `autoload` isn't supported (spike scope) |
| 39 | cannot load such file |
| 30 | unsupported string interpolation part (spike scope) |
| 24 | `require_relative` is only supported as a top-level statement with a single string-literal argument … |
| 19 | expected a constant name or path (e.g. `Foo` or `Foo::Bar`) |
...
| 4 | `def` with an explicit non-`self` receiver isn't supported yet (spike scope) |
...
| 3 | unknown class/module `Gem` in `Gem::UnknownCommandSpellChecker` (must be defined earlier in the file… |

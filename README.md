...
**185,647 of 195,778 probed gems compile to Rust (94.8%).** The probe runs zeo's full front end (parse, lower, analyze, codegen) on the newest release of every gem on rubygems.org. `ok` means zeo produced Rust; no rustc ran. One row per gem in [`conformance/gem-probe.tsv`](conformance/gem-probe.tsv).

| Verdict | Gems | Share of probed |
|---|---|---|
| Compile to Rust (`ok`) | 185,647 | 94.8% |
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 398 | 0.2% |
...
| No verdict reached (`timeout`, `out-of-memory`) | 58 | 0.0% |

Of the 187,435 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **185,647 compile (99.0%)**.
<!-- gem-probe-stats:end -->

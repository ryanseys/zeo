...
**183,679 of 195,778 probed gems compile to Rust (93.8%).** The probe runs zeo's full front end (parse, lower, analyze, codegen) on the newest release of every gem on rubygems.org. `ok` means zeo produced Rust; no rustc ran. One row per gem in [`conformance/gem-probe.tsv`](conformance/gem-probe.tsv).

| Verdict | Gems | Share of probed |
|---|---|---|
| Compile to Rust (`ok`) | 183,679 | 93.8% |
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 1,979 | 1.0% |
| Unresolved dependency in the probe's view (`missing-dependency`) | 1,329 | 0.7% |
| Harness limits, not compiler verdicts (`no-entry-point`, `no-lib-dir`, `fetch-failed`, `view-failed`, `ambiguous-require`) | 1,933 | 1.0% |
| Nothing to compile (`invalid-ruby`, `native-extension`, `ext-only`, `meta-gem`, `platform-gem`) | 6,811 | 3.5% |
| No verdict reached (`timeout`, `out-of-memory`) | 47 | 0.0% |

Of the 187,034 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **183,679 compile (98.2%)**.
<!-- gem-probe-stats:end -->

...
**183,217 of 195,778 probed gems compile to Rust (93.6%).** The probe runs zeo's full front end (parse, lower, analyze, codegen) on the newest release of every gem on rubygems.org. `ok` means zeo produced Rust; no rustc ran. One row per gem in [`conformance/gem-probe.tsv`](conformance/gem-probe.tsv).

| Verdict | Gems | Share of probed |
|---|---|---|
| Compile to Rust (`ok`) | 183,217 | 93.6% |
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 1,977 | 1.0% |
| Unresolved dependency in the probe's view (`missing-dependency`) | 1,328 | 0.7% |
| Harness limits, not compiler verdicts (`no-entry-point`, `no-lib-dir`, `fetch-failed`, `view-failed`, `ambiguous-require`) | 3,658 | 1.9% |
| Nothing to compile (`invalid-ruby`, `native-extension`, `ext-only`, `meta-gem`, `platform-gem`) | 5,551 | 2.8% |
| No verdict reached (`timeout`, `out-of-memory`) | 47 | 0.0% |

Of the 186,569 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **183,217 compile (98.2%)**.
<!-- gem-probe-stats:end -->

...
**171,223 of 195,778 probed gems compile to Rust (87.5%).** The probe runs zeo's full front end (parse, lower, analyze, codegen) on the newest release of every gem on rubygems.org. `ok` means zeo produced Rust; no rustc ran. One row per gem in [`conformance/gem-probe.tsv`](conformance/gem-probe.tsv).

| Verdict | Gems | Share of probed |
|---|---|---|
| Compile to Rust (`ok`) | 171,223 | 87.5% |
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 2,587 | 1.3% |
| Unresolved dependency in the probe's view (`missing-dependency`) | 1,277 | 0.7% |
| Harness limits, not compiler verdicts (`no-entry-point`, `no-lib-dir`, `fetch-failed`, `view-failed`, `ambiguous-require`) | 18,256 | 9.3% |
| Nothing to compile (`invalid-ruby`, `native-extension`, `ext-only`, `meta-gem`, `platform-gem`) | 2,410 | 1.2% |
| No verdict reached (`timeout`, `out-of-memory`) | 25 | 0.0% |

Of the 175,112 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **171,223 compile (97.8%)**.
<!-- gem-probe-stats:end -->

...
**170,213 of 195,778 probed gems compile to Rust (86.9%).** The probe runs zeo's full front end (parse, lower, analyze, codegen) on the newest release of every gem on rubygems.org. `ok` means zeo produced Rust; no rustc ran. One row per gem in [`conformance/gem-probe.tsv`](conformance/gem-probe.tsv).

| Verdict | Gems | Share of probed |
|---|---|---|
| Compile to Rust (`ok`) | 170,213 | 86.9% |
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 3,345 | 1.7% |
...
Of the 174,851 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **170,213 compile (97.3%)**.
<!-- gem-probe-stats:end -->

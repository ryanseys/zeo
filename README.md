...
**185,292 of 195,778 probed gems compile to Rust (94.6%).** The probe runs zeo's full front end (parse, lower, analyze, codegen) on the newest release of every gem on rubygems.org. `ok` means zeo produced Rust; no rustc ran. One row per gem in [`conformance/gem-probe.tsv`](conformance/gem-probe.tsv).

| Verdict | Gems | Share of probed |
|---|---|---|
| Compile to Rust (`ok`) | 185,292 | 94.6% |
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 788 | 0.4% |
...
| No verdict reached (`timeout`, `out-of-memory`) | 49 | 0.0% |

Of the 187,461 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **185,292 compile (98.8%)**.
<!-- gem-probe-stats:end -->

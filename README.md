...
**171,184 of 195,778 probed gems compile to Rust (87.4%).** The probe runs zeo's full front end (parse, lower, analyze, codegen) on the newest release of every gem on rubygems.org. `ok` means zeo produced Rust; no rustc ran. One row per gem in [`conformance/gem-probe.tsv`](conformance/gem-probe.tsv).

| Verdict | Gems | Share of probed |
|---|---|---|
| Compile to Rust (`ok`) | 171,184 | 87.4% |
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 2,626 | 1.3% |
...
Of the 175,112 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **171,184 compile (97.8%)**.
<!-- gem-probe-stats:end -->

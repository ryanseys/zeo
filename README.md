...
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 4,110 | 2.1% |
| Unresolved dependency in the probe's view (`missing-dependency`) | 1,967 | 1.0% |
| Harness limits, not compiler verdicts (`no-entry-point`, `no-lib-dir`, `fetch-failed`, `view-failed`, `ambiguous-require`) | 20,468 | 10.5% |
| Nothing to compile (`invalid-ruby`, `native-extension`, `ext-only`, `meta-gem`, `platform-gem`) | 845 | 0.4% |
| No verdict reached (`timeout`, `out-of-memory`) | 18 | 0.0% |

Of the 174,465 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **168,370 compile (96.5%)**.
<!-- gem-probe-stats:end -->

...
**168,980 of 195,778 probed gems compile to Rust (86.3%).** The probe runs zeo's full front end (parse, lower, analyze, codegen) on the newest release of every gem on rubygems.org. `ok` means zeo produced Rust; no rustc ran. One row per gem in [`conformance/gem-probe.tsv`](conformance/gem-probe.tsv).

| Verdict | Gems | Share of probed |
|---|---|---|
| Compile to Rust (`ok`) | 168,980 | 86.3% |
| Compiler gaps, zeo's to fix (`lowering-gap`, `compiler-panic`, `rustc-error`) | 3,810 | 1.9% |
| Unresolved dependency in the probe's view (`missing-dependency`) | 1,775 | 0.9% |
| Harness limits, not compiler verdicts (`no-entry-point`, `no-lib-dir`, `fetch-failed`, `view-failed`, `ambiguous-require`) | 20,241 | 10.3% |
...
Of the 174,584 gems zeo can attempt -- the probed set minus the harness limits and the gems no Ruby loads -- **168,980 compile (96.8%)**.
<!-- gem-probe-stats:end -->
